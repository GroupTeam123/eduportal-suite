...
function drawSectionTitle(doc: jsPDF, title: string, y: number, margin: number = 15): number {
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(safeTitle, margin + 8, y + 6);
...
function drawInfoCard(
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(100, 100, 100);
  doc.text(safeLabel, x + 8, y + height / 2 - 3);
  
  // Value
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(color.r, color.g, color.b);
  doc.text(safeValue, x + 8, y + height / 2 + 6);
}
...
export function generateAnnualReportPDF(reportData: ReportData): void {
  try {
    ...
    const safeTitle = reportData.title || 'Report';
    const safeGeneratedBy = reportData.generatedBy || 'System';
    const safeDate = reportData.date || new Date().toLocaleDateString();
    
    // Draw header
    let currentY = drawHeader(
      doc,
      safeTitle,
      `Generated by: ${safeGeneratedBy} | ${safeDate}`,
  ...
  // Save the PDF
  const fileName = `${safeTitle.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);
  } catch (error) {
